...
def expect_and_grad_minibatch(  # noqa: F811
    ...
    σ = vstate.samples
    σ_batches = σ.reshape(n_minibatches, -1, σ.shape[-1])
...
def _grad_expect_hermitian_minibatch_iterate(
    ...
    O_locs = jnp.asarray(O_locs).flatten()
    σ = σ_batches.reshape(-1, σ_batches.shape[-1])

    # Don't minibatch the backward pass
...
def grad_expect_hermitian_minibatch_backward(
    ...
    Ō_grad = vjp_fun(jnp.conjugate(jnp.squeeze(O_loc)) / n_samples_tot)[0]
